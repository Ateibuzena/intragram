...
correr gateway manualmente en gateway/
npm run start:dev

Desde navegador

API básica: abre http://localhost:3000/test y valida respuesta {"message":"ok"}.
Métricas: abre http://localhost:3000/metrics y valida que aparezcan métricas tipo http_request_duration_seconds y process_.
...
Grafana: abre http://localhost:3001, entra (normalmente admin/admin si no cambió), y confirma que el dashboard de NestJS carga datos.

Desde terminal

Levantar stack en prod: docker compose up -d --build.
Verificar contenedores: docker compose ps (deben estar Up).
Smoke test API: curl -i http://localhost:3000/test.
Validar métricas: curl -s http://localhost:3000/metrics | head -n 30.
Generar tráfico y revalidar: for i in {1..50}; do curl -s http://localhost:3000/test >/dev/null; done && curl -s http://localhost:3000/metrics | grep http_request_duration_seconds_count.
