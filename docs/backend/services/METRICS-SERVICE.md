...
- Prometheus recoge métricas del gateway y de los cuatro servicios backend a través de `/metrics`.
- La pila incluye node-exporter como integración de infraestructura.
- Grafana tiene dashboards personalizados para gateway, auth, users, posts y chat.
...
- dashboards: gateway, auth, users, posts, chat
