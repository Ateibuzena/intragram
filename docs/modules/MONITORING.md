...
La implementación actual se basa en Prometheus para la recolección de métricas, Grafana para paneles, exporters para telemetría de infraestructura y Alertmanager para el enrutamiento de alertas.
...
flowchart LR
  ...
  NE[Node Exporter]
  PEA[Postgres Exporter Auth]
  PEU[Postgres Exporter Users]
  PEC[Postgres Exporter Chat]
  NGE[Nginx Exporter]
  ...
  PR -->|scrape /metrics| NGE
  PR -->|scrape /metrics| PEA
  PR -->|scrape /metrics| PEU
  PR -->|scrape /metrics| PEC

  NGE -->|scrape stub_status| NG
  PEA --> ADB
  PEU --> UDB
  PEC --> CDB
...
## Stack de Monitoreo
### Prometheus
Prometheus está configurado como el recolector central de métricas. Realiza scrapes sobre servicios de aplicación y exporters de infraestructura, además de evaluar reglas de alerta.
...
- nginx-prometheus-exporter: métricas de Nginx mediante stub_status interno.
- postgres-exporter (uno por base de datos): métricas de ejecución de PostgreSQL para auth, users y chat.
...
- Nginx exporter: implementado.
- Exporters de PostgreSQL: implementados (auth/users/chat).
...
La telemetría a nivel Docker se representa mediante contenedores exporter (node/nginx/postgres), no mediante cAdvisor.
...
- ExporterDown: detecta fallos en exporters (node/nginx/postgres).

Enrutamiento de alertas:
- Prometheus envía alertas a Alertmanager.
- Alertmanager agrupa por alert/job/instance/severity.
- Los niveles warning y critical tienen diferentes repeat intervals.
- Se configura inhibición para que alertas critical supriman warnings equivalentes.
- El receptor de correo se configura mediante variables de entorno.
...
- Prometheus y Alertmanager no están expuestos en puertos públicos del host; el acceso se hace mediante proxy en Nginx.
...
| nginx-exporter | /metrics (scrapea Nginx stub_status) | Métricas de ejecución de Nginx |
| postgres-exporter-auth | /metrics | Métricas PostgreSQL de auth-db |
| postgres-exporter-users | /metrics | Métricas PostgreSQL de users-db |
| postgres-exporter-chat | /metrics | Métricas PostgreSQL de chat-db |
...
- Exporters/integraciones: implementados para node, nginx y PostgreSQL.
- Dashboards personalizados de Grafana: implementados por servicio con cobertura operativa.
- Reglas de alerta: implementadas con enrutamiento Prometheus + Alertmanager.
