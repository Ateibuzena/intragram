export default function MessageBubble({ message, showTimestamp }) {
      ...
      <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
        <div className={`max-w-xs ${isMe ? 'bg-blue-600' : 'bg-ft-card border border-ft-border'} rounded-2xl px-4 py-2.5`}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 h-8 bg-white/20 rounded-full overflow-hidden relative">
              <div className="h-full flex items-center px-2 gap-0.5">
                {Array.from({ length: 40 }).map((_, i) => (
                  <div
                    key={i}
                    className={`w-0.5 rounded-full ${isMe ? 'bg-white' : 'bg-ft-cyan'}`}
          ...
          {showTimestamp && (
            <p className={`text-[10px] ${isMe ? 'text-white/60' : 'text-ft-muted'} mt-1 text-right`}>
              Ver transcripción
            </p>
          )}
        </div>
