		...
		<div className="surface-glass border border-ft-border rounded-2xl p-4">
			...
			<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 text-xs">
				...
				<div className="min-w-0">
					...
					<p className="text-white truncate">{insights.campus}</p>
				</div>
				<div className="min-w-0">
				...
				<div className="min-w-0">
					...
					<p className="text-white truncate">{profile?.pool_month ?? 'N/A'} {profile?.pool_year ?? ''}</p>
				</div>
				<div className="min-w-0">
				...
				<div className="min-w-0">
					...
					<p className="text-white truncate">{profile?.forty_two_active === null || profile?.forty_two_active === undefined ? 'N/A' : profile.forty_two_active ? 'Yes' : 'No'}</p>
				</div>
				<div className="min-w-0">
				...
				<div className="min-w-0">
					...
					<p className="text-white truncate text-[11px]">{formatDate(insights.cursusBeginAt)}</p>
				</div>
				<div className="min-w-0">
					<p className="text-ft-muted uppercase text-[10px] font-semibold">Cursus End</p>
					<p className="text-white truncate text-[11px]">{formatDate(insights.cursusEndAt)}</p>
				</div>
				<div className="min-w-0">
					<p className="text-ft-muted uppercase text-[10px] font-semibold">Black Hole</p>
					<p className="text-white truncate text-[11px]">{formatDate(insights.cursusBlackholedAt)}</p>
				</div>
				<div className="min-w-0">
					<p className="text-ft-muted uppercase text-[10px] font-semibold">Role</p>
					<p className="text-white truncate">{profile?.staff ? 'Staff' : profile?.alumni ? 'Alumni' : 'Student'}</p>
				</div>
				<div className="min-w-0">
					<p className="text-ft-muted uppercase text-[10px] font-semibold">Last Login</p>
					<p className="text-white truncate text-[11px]">{formatDate(profile?.last_login_at ?? null)}</p>
				</div>
				<div className="min-w-0">
					<p className="text-ft-muted uppercase text-[10px] font-semibold">Created</p>
					<p className="text-white truncate text-[11px]">{formatDate(profile?.created_at ?? null)}</p>
				</div>
			</div>
		</div>
	);
